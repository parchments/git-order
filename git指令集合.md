...
#### ? ? 6、提交到暂存区： git add <pathspec>(如果单个文件就像index.html，如果多个文件就用“ . ”点的方式)
...
#### ? ? 8、提交内容到远程仓库： git push <remote> <branch>
...
#### ? ? 12、克隆仓库：git clone <repository>

#### ? ? 13、远程仓库重命名：git remote rename oldName newName(这步可改可不改)

#### ? ? 14、添加源仓库地址为远程仓库： git remote add <name> <remote>
...
#### ? ? 19、比较两个commit之间的差异： git diff <commit-id> <commit-id>

####   
**基础工作流**  
? ? 20、查看分支： git branch

#### ? ? 21、创建新分支： git branch <name>，新建完后要切换分支，然后add，commit，push，最后在github才有显示

#### ? ? 22、创建并切换分支： git checkout -b <name>

#### ? ? 23、删除本地分支： git branch -d <name>

#### ? ? 24、删除远程分支： git push -d <origin> <branch>

#### ? ? 25、切换分支： git checkout <name>

#### ? ? 26、删除某个文件： git rm -r --cached target ?删除target文件夹后commit，push到master

#### ?? ?master是装着没有bug的代码，dev分支是跟master一样的代码，但是允许报错

#### ? ? 27、合并分支：git merge <branch>
